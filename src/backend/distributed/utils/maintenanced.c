...
InitializeMaintenanceDaemonBackend(void)
{
	...
	if (!found || !dbData->daemonStarted)
	{
		...
		SafeStrcpy(worker.bgw_library_name,
				   sizeof(worker.bgw_library_name), "citus");
		SafeStrcpy(worker.bgw_function_name, sizeof(worker.bgw_library_name),
				   "CitusMaintenanceDaemonMain");

		worker.bgw_main_arg = ObjectIdGetDatum(MyDatabaseId);
		SafeMemcpy(worker.bgw_extra, sizeof(worker.bgw_extra), &extensionOwner,
				   sizeof(Oid));
